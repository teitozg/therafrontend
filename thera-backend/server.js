...
// Después de la configuración del pool, pero antes de las rutas
...
app.use(
  cors({
    origin: ["http://localhost:3000", "https://thera-rexi.vercel.app"],
    credentials: true,
  })
);

// Luego las rutas básicas (MOVER ESTAS AQUÍ)
app.get("/", (req, res) => {
  res.json({ message: "Thera Backend API is running" });
});
...
app.get("/test-db", async (req, res) => {
  try {
    ...
    const [rows] = await promisePool.query("SELECT NOW() as server_time");
    log(`Database connection successful. Server time: ${rows[0].server_time}`);
    res.json({
      success: true,
      result: rows[0],
      config: {
...
// El puerto y app.listen deben ser lo último
const PORT = process.env.PORT || 5001;
app.listen(PORT, () => {
  console.log(`Server is running on port ${PORT}`);
  console.log(`Health check available at: /health`);
  console.log(`Database test available at: /test-db`);
});
